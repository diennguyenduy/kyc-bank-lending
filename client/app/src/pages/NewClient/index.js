...
const Login = () => {
  ...
  const validateForm = useCallback(() => {
    if (
      clientData.name &&
      clientData.name.length > 0 &&
      clientData.address &&
      clientData.address.length > 0 &&
      clientData.dateOfBirth &&
      clientData.dateOfBirth.length > 0 &&
      clientData.idNumber &&
      clientData.idNumber.length > 0 &&
      clientData.login &&
      clientData.login.length > 0 &&
      clientData.password &&
      clientData.password.length > 5 &&
    ...
    if (validated && isLoading) {
      try {
        `${process.env.REACT_APP_API_BACKEND}`
          .post('/customer', qs.stringify(clientData))
          .then((res) => {
            console.log(res);
            if (res.status === 200) {
              ...
              console.log(
                'Oopps... something wrong, status code ' + res.status
              );
